...
**Scope:** One cohesive feature that makes Pistis's core promise — *every claim
...
Today Pistis emits each answer claim as a **verbatim sentence** extracted from a
...
## Component 3 — Reproducible honesty-eval CLI (`pistis/eval.py`)

`python -m pistis.eval [--snapshot PATH] [--json]` — mirrors the existing
...
(`README` "Trust report" section + `SESSION_HANDOFF`). Pistis is not an
