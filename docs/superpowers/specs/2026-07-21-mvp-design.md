# FinanceEngine MVP — Design (2026-07-21)
...
### 2.1 Corpus (`finance_engine/corpus`)
...
### 2.2 Retrieval (`finance_engine/index`)
...
### 2.3 Engine (`finance_engine/engine`) — the product
...
### 2.4 Providers (`finance_engine/providers`) — optional, off by default
> **Implementation status (as of 2026-07-23): NOT BUILT.** This subsection is
> aspirational design, not a description of shipped code. `finance_engine/providers/`
...
### 2.5 API + UI
- **FastAPI** (`finance_engine/api/app.py`): `POST /ask` → `AnswerCard | Abstention |
