...
*Uncertain / undecided in research:* privacy is flagged as a top user concern (83%) but the research does not specify FinanceEngine's data-handling model, and it does not resolve whether any personalisation over a user's own data would ever be offered. Treat as an open product decision (see §7).
...
| **MoneyHelper** (Money & Pensions Service, gov-backed) | Free, impartial official guidance — 700+ guides, calculators, helplines, webchat/WhatsApp; includes Pension Wise | Highly authoritative and trusted; human specialists; but static content, no AI answer layer | Not an AI engine; slow to navigate. Better *cited by* FinanceEngine than treated as a rival — but is also the obvious body to build an AI layer itself |
...
- Persistent, prominent disclaimer: *"FinanceEngine provides information and guidance, not regulated financial advice or a personal recommendation. It does not consider your individual circumstances. For advice tailored to you, speak to an FCA-authorised adviser."*
...
FinanceEngine's defining mechanism is a **default-deny honesty gate**: the system's default posture is *not to answer*, and it earns the right to answer only when it can attach grounded, reliable citations to the claims it is about to make.

**How it decides to answer vs defer (conceptual):**
- **Ground-first, not generate-first.** For a given question, FinanceEngine retrieves from a curated corpus of UK-authoritative sources (GOV.UK/HMRC, FCA, MoneyHelper/Pension Wise, reputable consumer bodies) *before* composing an answer. The answer is assembled from what the sources support, not the model's parametric memory.
...
- **Abstain when unground-able.** If the corpus does not cover the question, or coverage is stale/ambiguous, FinanceEngine states that plainly and defers — to MoneyHelper and/or an FCA-authorised adviser — rather than guessing. Refusal is the feature, not a failure mode.
...
**The calibration principle:** the gate must be *trustworthy without being useless*. Too eager to answer, and FinanceEngine inherits the hallucination problem that destroys its only USP; too eager to abstain, and it loses to a ChatGPT that always answers. Tuning this threshold is the product's core execution challenge (see §7), and the target is a defensible, honest calibration — not maximal answer coverage.
...
5. **Consumer Duty exposure** if FinanceEngine becomes/partners with an authorised firm — inaccurate info = foreseeable harm. (A purely unauthorised info service isn't directly under the Duty, but partnerships/promotions can pull it in.)
...
- **Advice classification is fact-specific.** Whether a *given* FinanceEngine feature is "guidance," "targeted support," or a "personal recommendation" turns on exact wording/UX and needs a compliance lawyer against PERG 8 Annex 1 worked examples — which the regulatory research could not fully extract (the page renders via JS; analysis relied on PERG 8.30B + FCA "Helping firms…" guidance instead).
- **Consumer Duty applicability** depends on FinanceEngine's authorisation/partnership structure, which is not specified — must be checked.
- **FG24/1 social-media detail** is summarised only at a high level here.
- **Privacy / personalisation model is undefined.** 83% of UK AI-money users worry about privacy, but the research does not specify FinanceEngine's data handling, nor resolve whether any personalisation over a user's own data is in scope. This is an open product decision with direct trust implications.
