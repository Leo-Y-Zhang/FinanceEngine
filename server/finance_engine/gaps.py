...
aggregates the concepts that make FinanceEngine abstain — the query terms no trusted
...
coverage), so this report cannot tell a real gap from a question FinanceEngine
...
Privacy posture — what is and is not guaranteed:
  ...
  * Deterministic and offline — no network, no keys. The report names its own
    ...
    python -m finance_engine.gaps                 # human-readable report
    python -m finance_engine.gaps --all           # list every concept above the floor
...
from finance_engine.models import Abstention, RoutingEvent

# Paths mirror finance_engine.api.app (kept local so this CLI does not import FastAPI).
...
def corpus_gap_report(
    ...
    that still make FinanceEngine abstain. Re-asking (rather than trusting the logged
...
def _format(r: GapReport) -> str:
    lines = [
        "FinanceEngine corpus-gap report",
        "========================",
    ...
    lines += [
        ...
        "uncovered concept may be a real corpus gap OR a question FinanceEngine",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="FinanceEngine corpus-gap report")
