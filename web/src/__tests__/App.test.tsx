...
const ABSTAIN: AskResponse = {
  ...
  reason: "The sources FinanceEngine trusts do not cover this well enough to answer reliably.",
  routing: {
  ...
  report: {
    stage: "weak_coverage",
    explanation:
      "FinanceEngine found related material but no trusted source covers 'weather', 'manchester' — the part it could not verify.",
    signals: [
...
const ABSTAIN_NO_DIAGNOSTICS: AskResponse = {
  ...
  report: {
    stage: "no_source",
    explanation: "No trusted source in FinanceEngine's corpus addresses this question.",
    signals: [],
    uncovered_terms: [],
  },
};
...
describe("App", () => {
  ...
  it("navigates to the privacy notice page and back via the hash route", async () => {
    ...
    await user.click(screen.getByRole("link", { name: /back to financeengine/i }));
